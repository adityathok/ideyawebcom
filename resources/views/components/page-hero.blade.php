...
<section {{ $attributes->merge(['class' => 'relative isolate overflow-hidden bg-[#0a1589]']) }}>
    <div aria-hidden="true" class="pointer-events-none absolute inset-0 -z-10 overflow-hidden">
        @if ($image)
            <img src="{{ $image }}" alt="" class="absolute inset-0 size-full object-cover object-center brightness-90" loading="eager" decoding="async" />
        @else
        ...
        {{-- Gradasi ke putih di dasar: transisi mulus ke section putih di bawah.
             Dilewati saat latar memakai gambar sendiri, supaya fotonya tidak terpotong putih. --}}
        @unless ($image)
