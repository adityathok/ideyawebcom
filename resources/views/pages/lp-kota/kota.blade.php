<x-layouts.public :title="'Jasa Website '.$namaKota" :seo-meta="$seoMeta">
    ...
    <x-page-hero
        :breadcrumb="$namaKota"
        :trail="[['name' => 'Wilayah Layanan', 'url' => route('lp.index')]]"
        :badge="'Wilayah '.$namaKota"
        :image="$gambarHero"
    >
        <x-slot:title>Jasa website &amp; web app di <span class="text-gradient-dark">{{ $namaKota }}</span></x-slot:title>
        <x-slot:subtitle>Kami mengerjakan website, web app custom, WordPress, dan maintenance untuk bisnis di {{ $namaKota }} — termasuk {{ $kecamatan->count() }} kecamatan di bawah ini.</x-slot:subtitle>
        <x-slot:actions>
    ...
    <section id="kecamatan" class="bg-white">
        <div class="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
            <div class="max-w-2xl">
                ...
                <p class="mt-4 text-[18px] leading-7 tracking-[-0.16px] text-[#65646e]">Pilih kecamatan untuk melihat halaman wilayahnya, lengkap dengan deskripsi dan foto.</p>
            </div>
            ...
            <div class="mt-12 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
                @foreach ($kecamatan as $item)
                    @php
                        ...
                        $urlKecamatan = route('lp.kecamatan', ['kota' => $kotaSlug, 'kecamatan' => $item->slugKecamatan()]);
                    @endphp
                    <article class="flex h-full flex-col overflow-hidden rounded-[16px] border border-[#e3eaff] bg-[#fafbff] transition duration-200 hover:-translate-y-0.5 hover:shadow-[0_2px_5px_#0a158933]">
                        ...
                        <div class="flex flex-1 flex-col p-6">
                            ...
                            @if (filled($item->deskripsi))
                                <p class="mt-3 flex-1 text-sm leading-6 text-[#65646e]">{{ \Illuminate\Support\Str::limit($item->deskripsi, 140) }}</p>
                            @else
                                <div class="flex-1"></div>
                            @endif

                            <a href="{{ $urlKecamatan }}" class="mt-5 inline-flex w-fit items-center rounded-[14px] bg-[#0a1589] px-4 py-2.5 text-sm font-medium leading-none text-white transition hover:bg-[#06105a]">
                                Lihat {{ $item->nama_kecamatan }} →
                            </a>
                        </div>
                    </article>
                @endforeach
        ...
        <div class="mx-auto max-w-7xl px-4 pb-24 sm:px-6 lg:px-8">
            <div class="relative isolate overflow-hidden rounded-[32px] border border-[#e3eaff] bg-[#f3f6ff] px-6 py-16 text-center sm:px-12 lg:py-20">
                ...
                <div class="mt-8 flex flex-col items-center justify-center gap-3 sm:flex-row">
                    ...
                    <a href="{{ route('lp.index') }}" class="inline-flex w-full items-center justify-center rounded-[20px] border border-[#e3eaff] bg-white px-8 pb-4 pt-[18px] text-[16px] font-medium leading-none text-[#100f12] transition hover:bg-[#fafbff] sm:w-auto sm:text-[18px]">Wilayah lain</a>
                </div>
            </div>
        </div>
    </section>
